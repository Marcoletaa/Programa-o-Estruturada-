...
void incluirUsuario() {
    ...
    novoUsuario.nome = malloc(MAX_LENGTH * sizeof(char));
    printf("Digite o nome do usuario: ");
    fgets(novoUsuario.nome, MAX_LENGTH, stdin);

    novoUsuario.email = malloc(MAX_LENGTH * sizeof(char));
    printf("Digite o e-mail do usuario: ");
    fgets(novoUsuario.email, MAX_LENGTH, stdin);
    
    novoUsuario.sexo = malloc(MAX_LENGTH * sizeof(char));
    printf("Digite o sexo do usuario: ");
    fgets(novoUsuario.sexo, MAX_LENGTH, stdin);
    
    novoUsuario.endereco = malloc(MAX_LENGTH * sizeof(char));
...
int main() {
    ...
    // Liberar memória alocada para os usuários
    int i;
    for (i = 0; i < totalUsuarios; i++) {
        free(usuarios[i].nome);
        free(usuarios[i].email);
